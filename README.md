# Running DClaw on Dedalus Cloud Services

## Overview

[OpenClaw](https://docs.openclaw.ai) is an open-source, self-hosted AI assistant with a WebSocket gateway, 50+ messaging integrations, multi-agent routing, and a browser Control UI. This spec covers deploying and operating it on [Dedalus Cloud Services](https://dcs.dedaluslabs.ai) machines -- isolated Linux microVMs (Ubuntu 24.04) with persistent storage.
...
| `StorageProvisioningFailed` | Dedalus machines capacity | Wait and retry |
...
| `memory_mib exceeds capacity` (400) | Requested memory exceeds tier limit | Request a smaller `memory_mib`, or use production tier |
